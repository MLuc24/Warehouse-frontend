...
class PDFService {
  ...
  async generateGoodsReceiptPDF(goodsReceipt: GoodsReceipt): Promise<Blob> {
    ...
    // Total amount
    const autoTablePdf = pdf as jsPDF & { lastAutoTable?: { finalY: number } }
    const finalY = (autoTablePdf.lastAutoTable?.finalY ?? 150) + 10
    pdf.setFontSize(12)
  ...
  async generateGoodsIssuePDF(goodsIssue: GoodsIssue): Promise<Blob> {
    const pdf = new jsPDF('p', 'mm', 'a4')
    pdf.setFont('helvetica')
    ...
    pdf.setFontSize(14)
    pdf.text(this.convertVietnameseText('PHIẾU XUẤT KHO'), 105, 30, { align: 'center' })
    
    // Issue basic info
    pdf.setFontSize(12)
    pdf.text(`${this.convertVietnameseText('Số phiếu')}: ${goodsIssue.issueNumber || this.convertVietnameseText('Chưa có')}`, 20, 50)
    
    const issueDate = goodsIssue.issueDate 
      ? new Date(goodsIssue.issueDate).toLocaleDateString('vi-VN')
      : this.convertVietnameseText('Chưa có')
    pdf.text(`${this.convertVietnameseText('Ngày tạo')}: ${issueDate}`, 20, 60)
    
    pdf.text(`${this.convertVietnameseText('Khách hàng')}: ${goodsIssue.customerName || ''}`, 20, 70)
    pdf.text(`${this.convertVietnameseText('Trạng thái')}: ${this.getStatusTextSimplified(goodsIssue.status || '')}`, 20, 80)
    
    if (goodsIssue.notes) {
      pdf.text(`${this.convertVietnameseText('Ghi chú')}: ${goodsIssue.notes}`, 20, 90)
    }
    ...
    const tableRows = goodsIssue.details?.map((item, index) => [
      (index + 1).toString(),
      item.productName || '',
      item.productSku || '',
      item.quantity?.toString() || '0',
    ...
    // Total amount
    const autoTablePdf2 = pdf as jsPDF & { lastAutoTable?: { finalY: number } }
    const finalY = (autoTablePdf2.lastAutoTable?.finalY ?? 150) + 10
    pdf.setFontSize(12)
    pdf.setFont('helvetica', 'bold')
    pdf.text(`${this.convertVietnameseText('Tổng tiền')}: ${goodsIssue.totalAmount?.toLocaleString('vi-VN') || '0'} ${this.convertVietnameseText('đ')}`, 140, finalY, { align: 'right' })
    ...
    pdf.text(`(${goodsIssue.createdByUserName || ''})`, 40, signatureY + 20, { align: 'center' })
  ...
  private getStatusTextSimplified(status: string): string {
    const statusMap: Record<string, string> = {
      ...
      'Approved': this.convertVietnameseText('Đã phê duyệt'),
      'SupplierConfirmed': this.convertVietnameseText('NCC đã xác nhận'),
      'InPreparation': this.convertVietnameseText('Đang chuẩn bị'),
      'ReadyForDelivery': this.convertVietnameseText('Sẵn sàng giao hàng'),
      'InTransit': this.convertVietnameseText('Đang vận chuyển'),
      'Delivered': this.convertVietnameseText('Đã giao hàng'),
  ...
  async downloadGoodsReceiptPDF(goodsReceipt: GoodsReceipt): Promise<void> {
    try {
      ...
      console.error('Error downloading GoodsReceipt PDF:', error)
      throw error
    }
  }

  async downloadGoodsIssuePDF(goodsIssue: GoodsIssue): Promise<void> {
    try {
      const pdfBlob = await this.generateGoodsIssuePDF(goodsIssue)
      const url = URL.createObjectURL(pdfBlob)
      const link = document.createElement('a')
      link.href = url
      link.download = `phieu-xuat-${goodsIssue.issueNumber || goodsIssue.goodsIssueId}.pdf`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
    } catch (error) {
      console.error('Error downloading GoodsIssue PDF:', error)
      throw error
    }
  }
  ...
  async downloadPDF(goodsReceipt: GoodsReceipt): Promise<void> {
    return this.downloadGoodsReceiptPDF(goodsReceipt)
  }
